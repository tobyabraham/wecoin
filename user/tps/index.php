...
if(isset($_SESSION['user'])){
    if(isset($_POST['search'])){
        $lkey = $_POST['search'];
    $querylicense = mysqli_query($conn,"SELECT * FROM `licensetable` WHERE `licensekey`='$lkey'");
    
    if(mysqli_num_rows($querylicense)>0){
        $lrow = mysqli_fetch_array($querylicense);
        if($lkey==$lrow['licensekey']){
            header("Location: ../tps/licensed.php"); 
        }
        else{
            echo '<script>alert("Invalid License Key!")</script>'; 
        }
      ...
      <input type="submit" value="Enter"  > </form>';
                               ...
                               <form method="post" action ="">
                                ...
                                if(isset($_POST['bothistory4']) || isset($_POST['tradebot4'])){
                                    ...
                                    <input name="search" placeholder="Enter License Key" required><input value="Enter"  type="submit" ></form>';
                                }
