...
if(isset($_SESSION['user'])){
    ...
    if(isset($_POST['options'])){
        ...
        $rs=mysqli_query($conn, "SELECT * FROM `botopenorder` WHERE `email`= '$email' LIMIT $nrows,1");
        $rowa = mysqli_fetch_array($rs);
        $openday = $rowa['Day'];
        
        $td = $rowa['Transaction ID'];
        $result = $rowa['Amount'];
        if($rowa['Trade Type']=="margin"){
        ...
        $querytrans = "UPDATE `botorderhistory` SET `Date`='$curdate',`Close Time`='$closedate',`Status`='$status',`Result`='$result',
        ...
        $queryopen = "DELETE FROM `botopenorder` WHERE `Transaction ID` = '$td'";
    ...
    if(isset($_POST['inserttrade'])){
        $data = array($_POST['inserttrade']);
        ...
        foreach($words as $multi){
        $tempss = explode(" ", $multi);
    ...
    for($i=0;$i<count($multiarray);$i++){
        $newvar = $multiarray[$i];
        if($newvar[2]=="margin"){
            if($newvar[0]>$_SESSION['marginbalance']){
                echo '<script >alert("Insufficient Balance")</script>';
            }
            if($newvar[0]<$_SESSION['marginbalance']){
                $_SESSION['marginbalance'] = $_SESSION['marginbalance'] - $newvar[0];
                $margbal = $_SESSION['marginbalance'];
                $result = $newvar[0]*$newvar[3];
                $finresult = $result - $newvar[0];
                $profit = array();
                $opendays = date("d");
                $loss = array();
                for ($x = 0; $x < 2; $x++) {
                ...
                $profitloss = array_merge($profit,$loss);
                shuffle($profitloss);
                $queryuse = "UPDATE `users` SET `margin balance`='$margbal' WHERE `email` = '$email'";
                mysqli_query($conn,$queryuse) or die("Could Not Perform the Query");
         
            $trid = rand(1111111111,9999999999);
        $querybot = "INSERT INTO `botopenorder`(`email`,`Transaction ID`,`Amount`, `Pair`, `Date`,`Open Time`,`Status`,`Trade Type`, `Leverage`, `Orders`,`Day`) 
        VALUES('$email','$trid','$newvar[0]','$newvar[1]','$currdate','$currdate','ACTIVE','$newvar[2]','$newvar[3]','$newvar[4]','$opendays')";
         $querybothistory = "INSERT INTO `botorderhistory`(`email`,`Transaction ID`,`Amount`, `Pair`, `Date`,`Open Time`,`Status`,`Trade Type`, `Leverage`, `Orders`,`Day`) 
         VALUES('$email','$trid','$newvar[0]','$newvar[1]','$currdate','$currdate','ACTIVE','$newvar[2]','$newvar[3]','$newvar[4]','$opendays')";
          mysqli_query($conn,$querybot) or die("Could Not Perform the Query");
          mysqli_query($conn,$querybothistory) or die("Could Not Perform the Query");       

          $ss=mysqli_query($conn, "SELECT * FROM `botopenorder` WHERE `Transaction ID` = '$trid'");
                $row = mysqli_fetch_array($ss);
                $openday = $row['Day'];
                if($opendays>($openday+6)){
                    $vs=mysqli_query($conn, "SELECT * FROM `botopenorder` WHERE `Transaction ID` = '$trid'");
       ...
       if($newvar[2]=="spot"){
        if($newvar[0]>$_SESSION['spotbalance']){
            echo '<script >alert("Insufficient Balance")</script>';
        }
        if($newvar[0]<$_SESSION['spotbalance']){
            $_SESSION['spotbalance'] = $_SESSION['spotbalance'] - $newvar[0];
            $margbal = $_SESSION['spotbalance'];
            $result = $newvar[0];
            ...
            $profitloss = array_merge($profit,$loss);
            shuffle($profitloss);
            $queryuse = "UPDATE `users` SET `spot balance`='$margbal' WHERE `email` = '$email'";
            mysqli_query($conn,$queryuse) or die("Could Not Perform the Query");
                        ...
                        <div class="css-egsqt7">
                            <div class="css-1sstzk2">
                                <div class="css-ekwbtp">
                                    <div data-bn-type="text" class="css-1260v1g">THIRD PARTY SOFTWARE</div>
                                </div>
                         
                             
                           
                            </div>
                            <div style="display:flex !important; justify-content: center; text-align:center;">
                            <form method="post" action ="">
                                
                                <h3 >Coinrule</h3>
                                <textarea name="inserttrade" style="width:700px; height:400px;" placeholder="Insert trades here" required></textarea>
                                <br>
                                <br>
                                <input style="background-color: darkgoldenrod; color: black; border-radius: 6px;"  value ="Insert Trade(s)" type="submit">
            ...
            <div class="col-md-12" >
                ...
                <div class="tab-content">
                    ...
                    echo "<table  class='table'>
                          ...
                          $ttp = " Trade Type";
                          $opt = "Options";
                          $querys = "SELECT * FROM `botopenorder` WHERE `email`='$email'  "; //You don't need a ; like you do in SQL
                          ...
                          echo" <th>" .$tid."</th> 
                                ...
                                while($row = mysqli_fetch_array($results)){
                                    ...
                                    <td><form method='post' action=''><label style='color:black;'>Close Trade</label><input style='color:black;'name='options' type='submit' value='$rows'></form></td>
                          ...
                          echo" <th>" .$tid."</th> 
                                ...
                                $queryhis = "SELECT * FROM `botorderhistory` WHERE `email`='$email' "; //You don't need a ; like you do in SQL
