...
if(isset($_SESSION["user"])){  
    ...
    $fname = $fname[0];
    $dollar=1 / $btc;
    $btcval = round( $dollar * $row['spot balance'] ,8 );
    $btcdemo = round( $dollar * $row['demo spot'] ,8 );
    $_SESSION['lastlogin'] = $row['last login'];

    if(isset($_POST["tmain"])){
        if($_POST["maininput"] > $_SESSION["spotbalance"]){
            echo '<script >alert("Insufficient Balance")</script>';
        }
        if($_POST["maininput"] < $_SESSION["spotbalance"]){
            $_SESSION["spotbalance"] = ($_SESSION["spotbalance"] - $_POST["maininput"]);
            $updatespotbal = ($_SESSION["spotbalance"]);
            $main = $_POST["maininput"];
            $query = "UPDATE `users` SET `spot balance`= '$updatespotbal', `main balance`=`main balance` + '$main' WHERE `email`='$email'";
    ...
    if(isset($_POST["tmarginmain"])){
        ...
        if($_POST["maininput"] < $_SESSION["spotbalance"]){
            ...
            $updatespotbal = ($_SESSION["spotbalance"]);
            $main = $_POST["maininput"];
            $query = "UPDATE `users` SET `spot balance`= '$updatespotbal', `margin balance`=`margin balance` + '$main' WHERE `email`='$email'";
            mysqli_query($conn,$query) or die("Could Not Perform the Query");
        }
    }
    if(isset($_POST["tdemo"])){
        if($_POST["demoinput"] > $_SESSION["demospot"]){
            echo '<script >alert("Insufficient Balance")</script>';
        }
        if($_POST["demoinput"] < $_SESSION["demospot"]){
            $_SESSION["demospot"] = ($_SESSION["demospot"] - $_POST["demoinput"]);
            $updatedemobal = ($_SESSION["demospot"]);
            $demospot = $_POST["demoinput"];
            $query = "UPDATE `users` SET `demo spot`= '$updatedemobal', `demo balance`=`demo balance` + '$demospot' WHERE `email`='$email'";
            mysqli_query($conn,$query) or die("Could Not Perform the Query");
        }
    }
    if(isset($_POST["tmargindemo"])){
        ...
        if($_POST["demoinput"] < $_SESSION["demospot"]){
            ...
            $updatedemobal = ($_SESSION["demospot"]);
            $demospot = $_POST["demoinput"];
            $query = "UPDATE `users` SET `demo spot`= '$updatedemobal', `demo margin`=`demo margin` + '$demospot' WHERE `email`='$email'";
            mysqli_query($conn,$query) or die("Could Not Perform the Query");
        }
    }
  ?>  
                                                            ...
                                                            <div class="css-vurnku">
                                                                <div class="css-1f978ju">
                                                                    <div class="css-ize0sl"><span data-bn-type="text"
                                                                            ...
                                                                            dir="ltr" class="css-1kbdyxh"style="text-align:right;"> Demo
                                                                            balance: </span></div>
                                                                    <div class="css-n5mzgu">
                                                                        <div dir="ltr" class="css-off8uh">
                                                                            <div data-bn-type="text"
                                                                                class="css-1t9tl2o"><span>$</span><?=$_SESSION['spotbalance']?><span style="font-size:12px;">&emsp;<?php echo $btcval; echo 'BTC';?></span>&emsp;&emsp;&emsp;&emsp;&emsp;</div>

                                                                                <div data-bn-type="text"
                                                                                class="css-1t9tl2o" style="text-align:right;"><span>$</span><?=$_SESSION['demospot']?><span style="font-size:12px;">&emsp;<?php echo $btcdemo; echo 'BTC';?></span></div>
